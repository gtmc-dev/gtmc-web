...
export async function approveRevisionAction(formData: FormData) {
  ...
  if (!adminUser?.githubPat) {
    throw new Error(
      "GitHub PAT not found. Please bind your PAT in your Profile Configuration first.",
    );
  }
  ...
  try {
    ...
    if (githubToken && repoOwner && repoName) {
      const response = await fetch(
        `https://api.github.com/repos/${repoOwner}/${repoName}/actions/workflows/submit_pr.yml/dispatches`,
        {
          method: "POST",
          headers: {
