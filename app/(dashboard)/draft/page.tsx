...
export default async function DraftDashboardPage() {
  ...
  const renderDraftCard = (draft: any) => (
    <BrutalCard
      key={draft.id}
      className="flex flex-col h-64 justify-between border border-tech-main/30 bg-white/60 backdrop-blur-sm p-6 relative group"
    >
      {/* 工业风边角 */}
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-4">
          <span
            className={`px-2 py-0.5 text-[10px] font-mono tracking-wider border ${
              draft.status === "DRAFT"
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            {draft.status !== "PENDING" && draft.status !== "APPROVED" && (
              ...
              >
                <button
                  type="submit"
                  className="text-[10px] uppercase font-mono text-red-500 hover:text-red-700 hover:underline"
                >
                  [DELETE]
                </button>
              </form>
      ...
      <Link href={`/draft/${draft.id}`} className="relative z-10 mt-auto">
        <BrutalButton
          variant="ghost"
          className="w-full text-xs font-mono tracking-widest h-10 border border-tech-main/20 hover:border-tech-main/50 bg-white/50 hover:bg-white/80 transition-all"
        >
    ...
    <div className="max-w-6xl mx-auto space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-tech-main/40 pb-6 relative">
        ...
        <Link href="/draft/new">
          <BrutalButton
            variant="primary"
            className="uppercase text-sm tracking-[0.1em] px-6 h-12 flex items-center justify-center hover:scale-[1.02] transition-transform"
          >
      ...
      <div className="space-y-8">
        <div>
          <h2 className="text-2xl font-bold uppercase tracking-widest text-tech-main-dark mb-6 border-b border-tech-main/20 pb-2">
            Active Records
          </h2>
